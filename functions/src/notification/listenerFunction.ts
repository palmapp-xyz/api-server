...
export function initNotifiers() {
  const onListingCreated = functions.firestore
    ...
    .onCreate(async (snapshot: QueryDocumentSnapshot) => {
      ...
      const deviceTokens: Record<string, DeviceToken> =
        await getMembersUserTokens({}, channelUrl, [
          order.order.maker,
          order.order.taker,
        ]);
      ...
      const initialReadStatus = Object.fromEntries(
        Object.keys(deviceTokens).map((userId) => [userId, false])
      );
      // store the notification in firestore, with empty read users list
      const added = await firestore
        .collection("notifications")
        .add({ ...payload, readStatus: initialReadStatus });
      ...
      for (let i = 0; i < chunks; i++) {
        const chunk = Object.values(deviceTokens).slice(
          i * batch,
          (i + 1) * batch
        );
        // eslint-disable-next-line no-await-in-loop
        const result: admin.messaging.MessagingDevicesResponse = await admin
          .messaging()
          .sendToDevice(
            chunk.flatMap((deviceToken: DeviceToken) =>
              deviceToken.apns.concat(deviceToken.fcm)
            ),
            payload,
            options
          );
  ...
  const onListingUpdated = functions.firestore
    ...
    .onUpdate(async (change: functions.Change<QueryDocumentSnapshot>) => {
      ...
      const deviceTokens: Record<string, DeviceToken> =
        await getMembersUserTokens({}, channelUrl, [
          afterOrder.order.maker,
          afterOrder.order.taker,
        ]);
