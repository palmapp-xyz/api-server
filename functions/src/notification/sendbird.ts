...
export async function fetchChannelMembers(
  ...
  let url = `${config.SENDBIRD_API_URL}/group_channels/${encodeURIComponent(
    channelUrl
  )}/members?limit=${LIMIT}`;
  if (nextCursor) {
    url += `&next=${nextCursor}`;
...
export const getMembersUserTokens = async (
  ...
  const { members, nextCursor } = await fetchChannelMembers(
    channelUrl,
    lastKey || undefined
  );
  ...
  if (userIds.length === 0) {
    functions.logger.log(
      `getMembersUserTokens: channelUrl ${channelUrl}: total: ${tokens.length}`
    );
    return tokens;
  }
  ...
  return getMembersUserTokens(
    Object.assign(tokens, fetched),
    channelUrl,
    excludeAddresses,
    nextCursor
  );
