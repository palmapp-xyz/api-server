...
export const deleteCollection = async (path: string): Promise<void> => {
  firestore.collection(path).listDocuments().then((docs: DocumentReference<DocumentData>[]) => {
...
export const getAllUsers = async (count: number, nextPageToken?: string | undefined): Promise<void> => {
  admin.auth().listUsers(100, nextPageToken)
      .then((listUsersResult) => {
        ...
        if (listUsersResult.pageToken) {
          ...
          Promise.all([deleteCollection('profiles'), deleteCollection('userProfiles')]);
        }
      })
      .catch((error) => {
